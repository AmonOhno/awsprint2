...
- **到達導線**: 400問中まだ数問しか図解が無く、ランダム出題ではまず当たらない。
  そのためホームに「図解つき問題 (N)」ボタンを置き、図解のある問題だけを出題できるようにしている。
  対象IDは `web/data/diagrams.js`(Web)と `Resources/diagram-index.json`(iOS)から読む。
- 図解のエッジ(矢印)ラベルは `wrappingWidth` の対象外で、長いと途中で折り返される。10文字程度までに抑える。
